import pandas as pd
import re
INPUT_PATH = "/Users/sahibjotsingh/Desktop/job-market-data-pipeline/data/raw/Software Engineer Salaries.csv"
OUTPUT_PATH = "/Users/sahibjotsingh/Desktop/job-market-data-pipeline/data/processed/jobs_cleaned.csv"
...
df["Company"]=df["Company"].str.replace('nan','data not available')
...
def extract_job(title):
    ...
    if ("software" in title or "backend" in title or "frontend" in title) and \
       ("engineer" in title or "developer" in title):
        return "software engineer"

    return "other"


df['job family']= df['Job Title'].apply(extract_job)
...
def extract_seniority(title):
    ...
    if('senior' in title or 'sr' in title or 'sr.' in title or re.search(r'\b(iii|iv|v|3|4)\b', title)):
        return 'senior'

    
    if('junior' in title or 'jr' in title or 'jr.' in title or 'entry' in title or re.search(r'\b(i|ii|1|2)\b', title)):
        return 'junior'
...
df['Location']= df['Location'].fillna('data not available')          
df[['city','state']] = df['Location'].str.split(',',n=1, expand=True)  
df['is_remote'] = df['Location'].str.contains('Remote')
df.loc[df['is_remote'], ['city','state']] = [None, None]
df['data_not_available'] = df['Location'].str.contains('data not available')
df.loc[df['data_not_available'],['city','state']] = [None,None]
...
def parse_salary(text):
    ...
    if text in ['nan', 'none', 'unavailable', '']:
        return None, None
